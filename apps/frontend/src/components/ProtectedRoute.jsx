...
export default function ProtectedRoute({ children, allowedRoles = [] }) {
    ...
    // 1. Si el estado aÃºn estÃ¡ cargando la sesiÃ³n inicial, no renderizamos nada (o un spinner)
    if (loading) {
    ...
    if (allowedRoles.length > 0 && !allowedRoles.includes(role)) {
        return (
            <div style={{ padding: 40, textAlign: 'center' }}>
                ...
                <p style={{ color: 'var(--tx2)', marginTop: 10 }}>Tu rol actual ('{role}') no tiene permisos para ver este mÃ³dulo.</p>
            </div>
        );
    }

    // 4. Si pasa las validaciones, renderizamos el mÃ³dulo
    return children;
}
